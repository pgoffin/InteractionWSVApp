...
// augemnted version with a relayout function
	...
	$.widget( 'aviz.sparklificator', {
		...
		**/
		options: {
			...
			environment: 'not_cloned',
			renderer: function(){},
		},


		// widgetEventPrefix: 'sparklificator:',
		...
		refresh: function(option) {
			...
			switch(option) {
				...
				case 'width':
					...
					var environment = $.isFunction(o.environment) ? o.environment.call(this, element[0]) : o.environment;

					o.renderer.call(this, sparklineSPAN, newWidth, newHeight, hoverInteraction, environment, data);
		...
		**/
		_setNewPosition: function(sparklineSPAN, sparklificatedSPAN, width, height) {
			...
			$('body').append(tmpNBSPNode);
			// sparklificatedSPAN.append(tmpNBSPNode);
			var lengthOfNBSP = tmpNBSPNode[0].offsetWidth;
			tmpNBSPNode.remove();

			// console.log('lengthOfNBSP: ' + lengthOfNBSP);
			// console.log('lengthOfEntity: ' + lengthOfEntity);
			...
			var diff = sparklineSPAN[0].getBoundingClientRect().left - $(sparklificatedSPAN).children('.entity')[0].getBoundingClientRect().right;
		...
		**/
		...
		/**
		* Stores the the options, destroys the sparklificator object and then rebuilds
		* it adapted to the new environemnt.
		**/
		relayout: function() {
